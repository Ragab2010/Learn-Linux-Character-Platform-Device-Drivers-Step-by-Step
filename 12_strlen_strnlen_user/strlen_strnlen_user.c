...
static ssize_t myRead(struct file *file, char __user *user_buffer, size_t len, loff_t *offset) {
    ...
    char kernel_buffer[]="hello";
    size_t data_len = strlen(kernel_buffer);  // actual data size
    int returnValue;
    pr_info("%s: Read operation\n", __func__);
    pr_info("%s: Kernel_Buffer:%p\t User_Buffer:%p\n", __func__, kernel_buffer, user_buffer);
    returnValue = copy_to_user(user_buffer ,kernel_buffer , data_len+1 );
    pr_info("strlen(kernel_buffer):%d\n", strlen(kernel_buffer));
...
static ssize_t myWrite(struct file *file, const char __user *user_buffer, size_t len, loff_t *offset) {
    ...
    int returnValue;
    returnValue = copy_from_user(kernel_buffer ,user_buffer , strnlen_user(user_buffer , 100) );
    pr_info("%s: Write operation\n", __func__);
    pr_info("%s: Kernel_Buffer:%p\t User_Buffer:%p\n", __func__, kernel_buffer, user_buffer);
    pr_info("strnlen_user(user_buffer , 100):%d\n", strnlen_user(user_buffer , 100));
