...
static ssize_t myRead(struct file *file, char __user *user_buffer, size_t len, loff_t *offset) {
    ...
    if(*offset + len > kernel_buffer_index){
        pr_err("%s: Attempt to read beyond valid data\n", __func__);
        len = kernel_buffer_index - *offset; // Adjust len to available data
    }
    //check if bytes_to_read is 0
    bytes_to_read = len;
    if (bytes_to_read == 0) {
...
static ssize_t myWrite(struct file *file, const char __user *user_buffer, size_t len, loff_t *offset) {
    ...
    if(*offset + len > MAX_SIZE){
        pr_err("%s: Attempt to read beyond valid data\n", __func__);
        len = MAX_SIZE - *offset; // Adjust len to available data
    }
    //check if bytes_to_read is 0
    bytes_to_write = len;
    if (bytes_to_write == 0) {
