...
static abc struct_kernel_buffer;
...
static ssize_t myRead(struct file *file, char __user *user_buffer, size_t len, loff_t *offset) {
    ...
    //now we read member by member  first the i
    returnValue = copy_to_user(&((abc * )user_buffer)->i ,&struct_kernel_buffer.i , sizeof(struct_kernel_buffer.i) );
    pr_info("copy_to_user for i member: return value :%d\n", returnValue);
    returnValue = copy_to_user(((abc * )user_buffer)->str,struct_kernel_buffer.str , sizeof(struct_kernel_buffer.str)+1 );
    pr_info("copy_to_user for str member: return value :%d\n", returnValue);
    pr_info("user_buffer len :%zu\n", len);
    pr_info("user_buffer offset :%llu\n", *offset);
    pr_info("the struct_kernel_buffer content: i=%d , str=%s\n", struct_kernel_buffer.i , struct_kernel_buffer.str);
    pr_info("----------------------------\n");
    return len;
}
...
static ssize_t myWrite(struct file *file, const char __user *user_buffer, size_t len, loff_t *offset) {
    ...
    returnValue = copy_from_user(&struct_kernel_buffer ,user_buffer ,len );
    pr_info("copy_from_user: return value :%d\n", returnValue);
    //now get the length of the string that str member point to , it store at the user space so we use strnlen_user instead of strlen
    //when we use copy_from_user(&struct_kernel_buffer ,user_buffer ,len ); we copy the address of string of heap of user space address in (struct_kernel_buffer.str)
    //so we use it to measure the length now by (struct_kernel_buffer.str)
    string_len =strnlen_user(struct_kernel_buffer.str , 100);
    //we copy_from_user from struct_kernel_buffer.str  to &struct_kernel_buffer the string_len
    returnValue = copy_from_user(&struct_kernel_buffer,struct_kernel_buffer.str ,string_len );
    pr_info("string len: strnlen_user(struct_kernel_buffer.str , 100) :%d\n", string_len);
    pr_info("user_buffer len :%zu\n", len);
    pr_info("user_buffer offset :%llu\n", *offset);
    pr_info("the struct_kernel_buffer content: i=%d , str=%s\n", struct_kernel_buffer.i , struct_kernel_buffer.str);
    pr_info("----------------------------\n");
